...
{\centering \includegraphics[width=0.9\textwidth]{figure/vif_mens_log-1} 

}

\caption[VIF Plot for the Men's Logistic Regression Model]{VIF Plot for the Men's Logistic Regression Model.}\label{fig:vif_mens_log}
\end{figure}
...
{\centering \includegraphics[width=0.9\textwidth]{figure/vif_womens_log-1} 

}

\caption[VIF Plot for the Women's Logistic Regression Model]{VIF Plot for the Women's Logistic Regression Model.}\label{fig:vif_womens_log}
\end{figure}
...
The \texttt{Elo\_diff} metric ranks second with $11.5\%$ of the total gain, capturing cumulative season long performance in a nuanced way that seeds alone cannot reflect. Together, these two features account for roughly $44\%$ of the model's total predictive power, forming the core of the algorithm's decision making process. The \texttt{avg\_point\_diff\_diff} variable contributes an additional $5.5\%$, reflecting the average margin by which each team outperformed its opponents during the regular season. Beyond these three primary features, the remaining predictors each contribute less than $3\%$ individually, with strength of schedule variables making up the next tier followed by a long tail of specific box score statistics. This concentration of predictive power in seeding and Elo suggests that broad measures of aggregate team quality drive tournament outcomes significantly more than any individual box score metric.
...
Variable & Estimate & p-value & Sig. \\ 
  \hline
Intercept & -207.3986 & 0.0000 & *** \\ 
  Off Rating & 0.6519 & 0.0000 & *** \\ 
  Def Rating & 0.6421 & 0.0000 & *** \\ 
  AdjT Team & 1.0826 & 0.0000 & *** \\ 
  AdjT Opp & 1.0086 & 0.0000 & *** \\ 
  Diff NetRtg & 0.0540 & 0.0029 & ** \\ 
  Diff Elo & 0.0013 & 0.1229 &  \\ 
   \hline
\end{tabular}
\end{table}
% latex table generated in R 4.4.2 by xtable 1.8-4 package
...
Variable & Estimate & p-value & Sig. \\ 
  \hline
Intercept & -202.9007 & 0.0000 & *** \\ 
  Off Rating & 0.7309 & 0.0000 & *** \\ 
  Def Rating & 0.7866 & 0.0000 & *** \\ 
  AdjT Team & 0.9605 & 0.0000 & *** \\ 
  AdjT Opp & 0.8542 & 0.0000 & *** \\ 
  Diff Elo & 0.0024 & 0.0000 & *** \\ 
   \hline
\end{tabular}
\end{table}
...
{\centering \includegraphics[width=0.9\textwidth]{figure/vif_mens-1} 

}

\caption[VIF Plot for the Men's Linear Regression Model]{VIF Plot for the Men's Linear Regression Model}\label{fig:vif_mens}
\end{figure}
...
{\centering \includegraphics[width=0.9\textwidth]{figure/vif_womens-1} 

}

\caption[VIF Plot for the Women's Linear Regression Model]{VIF Plot for the Women's Linear Regression Model}\label{fig:vif_womens}
\end{figure}
