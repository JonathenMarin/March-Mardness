...
cat("Regular season rows:", nrow(regular_data), "\n")
cat("Tournament rows:", nrow(tourney_data), "\n\n")


# ── Seed Features ─────────────────────────────────────────────────────────────

seeds[, seed_num := as.integer(gsub("[^0-9]", "", Seed))]
seeds_T1 <- copy(seeds[, .(Season, TeamID, seed_num)])
seeds_T2 <- copy(seeds[, .(Season, TeamID, seed_num)])
...
p1 <- ggplot(seed_summary, aes(x = T1_seed, y = avg_point_diff, color = factor(men_women))) +
  ...
  geom_ribbon(aes(ymin = avg_point_diff - sd_point_diff,
              ...
              alpha = 0.2, color = NA) +
  scale_color_manual(values = c("0" = "blue", "1" = "red"), labels = c("Women", "Men")) +
  scale_fill_manual(values  = c("0" = "blue", "1" = "red"), labels = c("Women", "Men")) +
  labs(title = "Average Point Differential by Team 1 Seed",
       x = "Team 1 Seed", y = "Average Point Differential",
...
p2 <- ggplot(seed_diff_summary[abs(Seed_diff) <= 15],
  ...
  labs(title    = "Average Point Differential by Seed Difference",
       ...
       x = "Seed Difference (T2_seed - T1_seed)", y = "Average Point Differential",
       color = "Division", fill = "Division") +
  theme_minimal()
...
plot_mens_elo <- ggplot(top_elo_mens, aes(x = TeamName, y = Elo_final, fill = Elo_final)) +
  ...
  coord_flip() +
  labs(title = paste("Top 20 Men's Teams", latest_season), x = NULL, y = "Elo Rating") +
  theme_minimal() +
  theme(axis.text = element_text(size = 10), plot.title = element_text(size = 13, face = "bold"),
...
plot_womens_elo <- ggplot(top_elo_womens, aes(x = TeamName, y = Elo_final, fill = Elo_final)) +
  ...
  scale_fill_gradient(low = "#56B1F7", high = "#132B43") +
  coord_flip() +
  labs(title = paste("Top 20 Women's Teams", latest_season), x = NULL, y = "Elo Rating") +
  theme_minimal() +
  theme(axis.text = element_text(size = 10), plot.title = element_text(size = 13, face = "bold"),
        legend.position = "none")
...
elo_T1 <- copy(elo_ratings); setnames(elo_T1, c("TeamID", "Elo_final"), c("T1_TeamID", "T1_Elo"))
elo_T2 <- copy(elo_ratings); setnames(elo_T2, c("TeamID", "Elo_final"), c("T2_TeamID", "T2_Elo"))
...
tourney_with_stats <- merge(tourney_with_stats, simple_stats,
                            by.x = c("Season", "T1_TeamID"), by.y = c("Season", "TeamID"), all.x = TRUE)
...
tourney_with_stats <- merge(tourney_with_stats, simple_stats,
                            by.x = c("Season", "T2_TeamID"), by.y = c("Season", "TeamID"), all.x = TRUE)
setnames(tourney_with_stats,
         c("games_played", "win_pct", "avg_score", "avg_opp_score", "avg_point_diff"),
...
tourney_with_stats <- merge(tourney_with_stats, elo_T1, by = c("Season", "T1_TeamID"), all.x = TRUE)
tourney_with_stats <- merge(tourney_with_stats, elo_T2, by = c("Season", "T2_TeamID"), all.x = TRUE)
...
create_clean_stats <- function(reg_data, metrics) {
  ...
  stats <- reg_data[, lapply(.SD, mean), by = .(Season, T1_TeamID), .SDcols = cols_to_avg]
...
opp_lookup <- merge(
  ...
  team_stats_clean,
  by.x = c("Season", "T2_TeamID"), by.y = c("Season", "TeamID")
)
...
final_profiles <- merge(team_stats_clean, sos_stats,
                        by.x = c("Season", "TeamID"), by.y = c("Season", "T1_TeamID"))
...
exclude_cols <- c("Season", "T1_TeamID", "T2_TeamID", "win", "PointDiff", "T1_games", "T2_games")
...
cat("Best CV Brier Score:", round(best_brier, 5), "\n")
...
cat("Men's 2026 teams:", length(mens_team_ids_2026), "\n")
cat("Women's 2026 teams:", length(womens_team_ids_2026), "\n")

mens_matchups   <- as.data.table(expand.grid(T1_TeamID = mens_team_ids_2026,   T2_TeamID = mens_team_ids_2026))
womens_matchups <- as.data.table(expand.grid(T1_TeamID = womens_team_ids_2026, T2_TeamID = womens_team_ids_2026))
mens_matchups   <- mens_matchups[T1_TeamID < T2_TeamID]
womens_matchups <- womens_matchups[T1_TeamID < T2_TeamID]

all_matchups <- rbind(mens_matchups, womens_matchups)
all_matchups[, Season    := 2026]
all_matchups[, men_women := as.integer(T1_TeamID < 3000)]

cat("Total matchups:", nrow(all_matchups), "\n")

test_set <- copy(all_matchups)
...
# ── Seeds — 2026 seeds already have the right season ─────────────────────────

seeds_2026_T1 <- seeds[Season == 2026, .(T1_TeamID = TeamID, T1_seed = as.integer(gsub("[^0-9]", "", Seed)))]
seeds_2026_T2 <- seeds[Season == 2026, .(T2_TeamID = TeamID, T2_seed = as.integer(gsub("[^0-9]", "", Seed)))]

test_set <- merge(test_set, seeds_2026_T1, by = "T1_TeamID", all.x = TRUE)
test_set <- merge(test_set, seeds_2026_T2, by = "T2_TeamID", all.x = TRUE)
...
test_set <- merge(test_set, t2_prof_test, by.x = "T2_TeamID", by.y = "TeamID", all.x = TRUE)

test_set[, Season := 2026]

cat("Columns in test set:", ncol(test_set), "\n")
cat("Rows before feature filter:", nrow(test_set), "\n")
...
d2026      <- xgb.DMatrix(data = as.matrix(valid_test[, ..features]))
probs_2026 <- predict(final_model, d2026)

if (min(probs_2026) < 0 || max(probs_2026) > 1) {
  probs_2026 <- 1 / (1 + exp(-probs_2026))
}

submission_2026 <- data.table(
  ID   = paste(2026, valid_test$T1_TeamID, valid_test$T2_TeamID, sep = "_"),
  Pred = probs_2026
)

write.csv(submission_2026, "Excel_Files/submission_2026_model2.csv", row.names = FALSE)

cat("Submission written:", nrow(submission_2026), "rows\n")
cat("Pred range: [", round(min(probs_2026), 4), ",", round(max(probs_2026), 4), "]\n")