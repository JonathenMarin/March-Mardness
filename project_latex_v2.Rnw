...
These three assumptions are assessed jointly through the combined diagnostic plots presented in Figures \ref{fig:diag_mens} and \ref{fig:diag_womens}. The first assumption, linearity, requires that the relationship between the dependent variable and each independent variable is linear. The Residuals vs. Fitted panels show residuals randomly scattered around zero with approximately flat smoothing lines across the range of predicted scores, confirming that the linearity assumption is reasonably satisfied in both models. The second assumption, normality of residuals, requires that the residuals follow an approximately normal distribution. The Q-Q panels show that the residuals follow the theoretical normal reference line closely through the central range in both models, with some deviation in the tails driven by a small number of extreme blowout games. The third assumption, homoscedasticity, requires that the variance of the residuals is consistent across all levels of the predicted values. The Residuals vs. Fitted panels appears to indicate the presence of heteroskedasticity. Performing classical tests such as the Breusch-Pagan test result in significant p-values. However, these same tests can be misleading for large sample sizes and the presence of outliers and high leverage points \cite{Pechcinska}. Satisfied with the visual inspection of the Residuals vs. Fitted plot and the existence of high leverage points in the Residuals vs. Leverage plot, we decided to proceed. 
...
\bibitem{Pechcinska}Pechci\'nska, A. (2023). \textit{The Robustness of Heteroscedasticity Tests}. Master's Thesis, Erasmus School of Economics, Erasmus University Rotterdam. Retrieved from \texttt{https://thesis.eur.nl/pub/70292/Thesis\_AgnieszkaPechcinska\_638592.pdf}
