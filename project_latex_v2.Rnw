...
\begin{align}
\text{Points}_A &\sim \mathcal{N}(\hat{\mu}_A, \hat{\sigma}^2) \\
\text{Points}_B &\sim \mathcal{N}(\hat{\mu}_B, \hat{\sigma}^2)
\end{align}
